...
def NoBrainer6():
    ...
    q = tk.Label(root, text="다음의 설명을 보고 알맞는 답을 제출하시오.")
    d = tk.Label(root, text="UI 유형 중 그래픽 환경을 기반으로 한 마우스나 전자펜을 사용하는 사용자 인터페이스이다.")
...
def NoBrainer7():
    ...
    q = tk.Label(root, text="다음 프로토콜의 번호를 작성하시오.")
    d = tk.Label(root, text="SSH")
...
def NoBrainer8():
    ...
    def check_answer():
        user_input = entry.get()
        correct_answers = {"Dos", "Denial of Service"} 
        global score
    ...
    q = tk.Label(root, text="다음의 설명을 보고 답을 작성하시오.")
    d = tk.Label(root, text="'이것'은 시스템을 악의적으로 공격해 자원을 부족하게 하여 사용하지 못하게 하는 공격이다.")
